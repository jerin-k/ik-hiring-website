...
export function renderInterviewer(data, opts = {}) {
  ...
  if (!ivs.length) {
    return `
      <div class="card" style="text-align:center;padding:2.5rem;">
        <p style="color:var(--muted);font-size:0.8125rem;">Interviewer data is being generated by the pipeline.</p>
        <p style="color:var(--muted);font-size:0.6875rem;margin-top:0.375rem;">It appears here after the next data refresh completes.</p>
      </div>`;
  }
  ...
  return `
    <style>
      ...
      .iv-filters select { appearance:none; -webkit-appearance:none; height:1.75rem; padding:0 1.75rem 0 0.6875rem; border:1px solid var(--border);
        border-radius:0.5rem; font-size:0.75rem; font-weight:500; background:var(--card); color:var(--text); cursor:pointer;
      ...
      .iv-filters select:focus { outline:none; border-color:var(--accent); box-shadow:0 0 0 0.1875rem rgba(78,107,166,0.16); }
      .iv-filters .fchip { display:flex; align-items:center; gap:0.4375rem; }
      ...
      .iv-grain { font-size:0.6875rem; color:var(--orange); margin:-0.5rem 0 0.875rem; }
      /* Chart spans the full content width rather than sitting in a narrow centred column. */
      .iv-chart-wrap { width:100%; border:1px solid var(--border); border-radius:0.625rem; background:var(--card);
        padding:0.875rem 1rem; margin:0 0 1.5rem; position:relative; }
    ...
    <div class="iv-filters">
      ...
      <div class="fchip"><label class="opt" style="font-size:0.75rem;font-weight:500;display:flex;align-items:center;gap:0.25rem;cursor:pointer"><input type="checkbox" id="ivExpandAll" checked> Expand all</label></div>
    </div>`}
    ...
    <p class="sub-note" style="margin-top:-0.25rem" id="ivChartNote"></p>
    <div class="iv-chart-wrap" id="ivChartWrap"><canvas id="ivTopChart"></canvas></div>

    <h3 class="subsection-title">Department → Panelist → Job <span id="ivRowCount" style="font-weight:400;color:var(--muted);font-size:0.6875rem"></span></h3>
...
export function initInterviewer(data, opts = {}) {
  ...
  function render() {
    ...
    if (distinct == null) {
      ...
      setText('ivCardIntSub', `${people.size} panelist${people.size === 1 ? '' : 's'} in view`);
    } else {
    ...
    if (body) {
      ...
      dNames.forEach((dept, di) => {
        ...
        html += `<tr data-path="${di}" data-haschild data-exp="0" style="cursor:pointer;background:var(--border-light)">
          <td style="font-weight:600">${CARET}${esc(dept)}<span style="color:var(--muted);font-weight:400;font-size:0.6875rem;margin-left:0.375rem">${pNames.length}</span></td>
          <td style="font-weight:600">${D.total}</td>
          <td class="${dCov != null ? cls(dCov) : ''}" style="font-weight:600">${dCov != null ? `${dCov}% <span class="cov-n">(${(dAgg.life - dAgg.pend).toLocaleString()}/${dAgg.life.toLocaleString()})</span>` : '—'}</td>
          <td style="font-weight:600" class="${dTurn != null && dTurn > 72 ? 'warn' : ''}">${fmtTurn(dTurn)}</td></tr>`;
        pNames.forEach((nm, pi) => {
          ...
          html += `<tr data-path="${di}-${pi}" data-haschild data-exp="0" style="display:none;cursor:pointer">
            <td style="padding-left:1.875rem;font-weight:500">${CARET}${esc(nm)}<span style="color:var(--muted);font-weight:400;font-size:0.6875rem;margin-left:0.375rem">${P.jobs.length}</span></td>
            <td>${P.total}</td>
          ...
          P.jobs.slice().sort((a, b) => b._count - a._count).forEach((j, ji) => {
            html += `<tr data-path="${di}-${pi}-${ji}" style="display:none">
              <td style="padding-left:3.625rem;color:var(--muted)">${esc(j.jobTitle || '(untitled)')}</td>
              <td>${j._count}</td><td></td>
              <td class="${j.avgTurnaroundHrs != null && j.avgTurnaroundHrs > 72 ? 'warn' : ''}">${fmtTurn(j.avgTurnaroundHrs)}</td></tr>`;
          });
        });
      });
      body.innerHTML = html || `<tr><td colspan="4" style="text-align:center;color:var(--muted);padding:1.125rem;font-size:0.75rem">No panelist ran an interview under these filters.</td></tr>`;
      wireIvTree(body, (F && F.expandAll) ? !!F.expandAll() : !!document.getElementById('ivExpandAll')?.checked);
    }
  ...
  function buildChart(rows, months, label) {
    ...
    if (!top.length) { if (wrap) wrap.style.height = '8.75rem'; return; }
...
function wireIvTree(tbody, expand) {
