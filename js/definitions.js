    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          ['Joined — Current Qtr Openings <span class="defs-tag">Sales · Others</span>', 'Everyone else who joined — <strong>Joined Total minus the column beside it</strong>, so the two always add up. Because it is a subtraction it also holds <strong>every joiner with no known opening</strong> (none on the offer, and none found on the hire). Those people are listed by name on <strong>Data Hygiene ➔ Hired Missing Opening Link</strong>, which is where the link can be fixed. So this column means <em>“not known to be earlier”</em>, not <em>“raised this quarter”</em>.'],
          ['Joined total <span class="defs-tag">Sales · Others</span>', 'Everyone who joined, before the split beside it — <strong>exactly the two Joined columns added</strong>. It exists only on the <strong>Sales</strong> and <strong>Others</strong> tables, where Joined is split by whether the position was opened in an earlier quarter or this one (#39); <strong>Non-Sales</strong> shows a single Joined pair instead.'],
          ['JP Total <span class="defs-tag">heads and score</span>', 'Always <strong>exactly the two columns beside it added together</strong>, in <strong>both</strong> halves — never counted separately. <strong>Its Score was missing until 25 Sep 2026</strong>: the figure was always in the data, the cell simply was not drawn (Jerin: <em>“Wasnt deliberate”</em>), and Overall Efficiency had shown both all along. On <strong>Sales</strong> and <strong>Others</strong> that is everyone currently in Ref Check, Documentation or Offer. On <strong>Non-Sales</strong> it leaves out anyone on an earlier quarter’s opening, and anyone joining next quarter on an opening that is not this quarter’s. Under the Score, a <strong>N no score</strong> caption counts how many of those same people scored <strong>nothing</strong> — their offer names <strong>no opening</strong>, or the opening it names has <strong>no Complexity</strong>. It is a count of the people already in the Heads figure beside it, never an estimate, and it is shown on the <strong>total only</strong>, not on the sub-columns.'],
          ...
          ['Delta', 'Goal minus what was achieved (Joined + Joining Pending on Non-Sales, Joined on Sales and Others), so it is the shortfall. Never shown below zero. On the Score side the bar fills with the shortfall. The <strong>+N sourced</strong> line underneath is the same sum for what the person sourced: the openings they are tagged on as Sourcer, minus the sourced people counted as achieved — also never below zero.'],
